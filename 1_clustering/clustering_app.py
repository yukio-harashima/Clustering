...
# 従来のウォード法に加え、SHC（安定クラスタリング）機能を単一ファイルに統合。

print("Clustering App script started. For log confirmation.")
...
class StableHierarchicalClustering:
    """
    Implementation of the SHC algorithm proposed in the paper "Stabilizing Hierarchical Clustering".

    Args:
        alpha (float): Sensitivity parameter for similarity calculation (α).
        lambda_ (float): Stability control parameter (λ). Using lambda_ as lambda is a reserved keyword in Python.
        max_depth (int): Maximum depth of the clustering (D).
    ...
    def fit(self, X: np.ndarray):
        """
        Executes the SHC algorithm on the dataset X and builds the linkage matrix.

        Args:
            X (np.ndarray): The dataset (n_samples, n_features).

        Returns:
            np.ndarray: A SciPy-compatible linkage matrix.
        """
        ...
        # Step 1: Build the hierarchical tree structure
        ...
        # Step 2: Convert the tree structure to a linkage matrix
        if root_node is None or (root_node.left is None and root_node.right is None):
            return np.array([]) # No clustering was formed
        ...
        for node in internal_nodes:
            if node.left.left is None and node.left.right is None: # left child is a leaf
                left_id = float(node.left.id)
            else: # left child is an internal node
                left_id = float(node_to_new_id[id(node.left)])

            if node.right.left is None and node.right.right is None: # right child is a leaf
    ...
    def _build_similarity_graph(self, X: np.ndarray):
        """Builds the similarity graph W based on the chosen metric."""
        if self.metric == 'euclidean':
            # Calculate squared Euclidean distance
            d_sq = squareform(pdist(X, 'sqeuclidean'))
        elif self.metric == 'cosine':
            # 1. Calculate cosine distance (1 - similarity)
            # pdist 'cosine' already returns 1 - cos(theta)
    ...
    def _form_tree_from_leaves(self, V_sub: list, current_depth: int):
        """Forcibly builds a tree structure from a list of leaf nodes."""
        if not V_sub: return None
    ...
    def _stable_sparsest_cut(self, V_sub: list):
        ...
        combinations_iter = itertools.combinations(V_sub, 2)
        num_combinations = (len(V_sub) * (len(V_sub) - 1)) // 2
        
        show_pbar = num_combinations > 1000 
        pbar_desc = f"SSC on {len(V_sub)} pts"
        
        if show_pbar:
            combinations_iter = tqdm(combinations_iter, total=num_combinations, desc=pbar_desc, leave=False)

        for i, j in combinations_iter:
            S_ij = {k for k in V_sub if self._W[i, k] > self._W[j, k]}
            
            size_S = len(S_ij)
            size_S_complement = len(V_sub) - size_S
            
            if size_S == 0 or size_S_complement == 0:
                phi = np.inf
            else:
                S_complement_set = set(V_sub) - S_ij
        ...
        return candidate_splits[chosen_index]
    ...
    with open(filepath, 'w', encoding='utf-8') as f:
        ...
        if config:
            display_order = [
                ...
                'shc_alpha', 'shc_lambda_', 'shc_n_clusters'
            ]
...
def stage_shc_clustering_and_labeling(data_for_shc, config, output_dir):
    ...
    print(f"  Parameters: alpha={alpha}, lambda={lambda_}, max_depth={max_depth}, metric={metric}")
    
    shc = StableHierarchicalClustering(alpha=alpha, lambda_=lambda_, max_depth=max_depth, metric=metric)
...
def run_all_combinations(base_working_dir, df_snap_orig, snap4_orig):
    ...
    # This function is kept as is, focusing on Ward's method as SHC requires interactive input.
    # A non-interactive version of SHC would require pre-defined cluster counts.
    pass
...
def main():
    ...
    if run_mode == '1':
        ...
        # Batch mode currently does not support the interactive part of SHC.
