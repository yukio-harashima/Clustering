#!/usr/bin/env python3
# -*- coding: utf-8 -*-
# PDTIによって出力されたデータsnap.datを使ってクラスタリングを行うスクリプト（軽量版）
#
# --- オミットされた機能 ---
# 以下の機能は未実装のため、この軽量版からは削除されています:
# 1. データフィルタリング手法:
#    - skewness_kurtosis (ヒストグラムの歪度と尖度で決定)
# 2. クラスター距離しきい値決定手法:
#    - threshold_elbow (エルボー法決定)
#    - threshold_hybrid (ハイブリッド決定)
# --------------------------

print("スクリプトが開始されました。ログ確認用。") 
...
def generate_output_dirname_from_config(config, arbitrary_threshold_val=None):
    ...
    else: # skewness_kurtosis は削除されたので、ここは実質通らない
        parts.append(f"filt-{filter_method[:3]}")

    var_method = config.get('variable_selection_method', 'none')
    if "matrix_components_" in var_method:
        suffix = var_method.replace("matrix_components_", "")
        parts.append(f"var-mat{suffix.upper()[:4]}")
    elif "nv6_components_" in var_method:
        suffix = var_method.replace("nv6_components_", "")
        parts.append(f"var-nv6{suffix.upper()[:4]}")
    else:
    ...
    # threshold_elbow, threshold_hybrid は削除されたので、ここは実質通らない
...
def read_run_pdti_ta_params(working_directory):
    ...
    if not os.path.exists(bash_file_path):
        ...
        while True:
            ...
            if not new_path:
                raise FileNotFoundError("run_PDTI_TA.bash が見つからず、パラメータ読み取りをスキップしました。")
            if os.path.exists(new_path):
                bash_file_path = new_path
                break
            else:
                print(f"指定されたパス '{new_path}' は存在しません。")
...
def stage_load_data(wd, snap_y_file="snap_y.dat", n_vector_file="n_vector.dat"):
    ...
    if not os.path.exists(ps_snap):
        raise FileNotFoundError(f"データファイルが見つかりません: {ps_snap}")
    ...
    if not os.path.exists(ps_nvector):
        raise FileNotFoundError(f"データファイルが見つかりません: {ps_nvector}")
...
def stage_filter_data(snap4_df, method, quantile_val=0.75, advanced_filter_params=None, output_dir_for_adv_details=None):
    """データフィルタリング"""
    print(f"\n--- 2. データフィルタリング ---")
    print(f"手法: {method}")
    ...
    # skewness_kurtosis は削除
        ...
        df_processed, filtered_df = stage_advanced_filtering(
            snap4_df.copy(), 
        ...
        if output_dir_for_adv_details: # 詳細保存オプション
            adv_details_path = os.path.join(output_dir_for_adv_details, "advanced_filtering_details.csv")
            df_processed.to_csv(adv_details_path, index=False)
            print(f"  高度フィルタリングの詳細情報を {adv_details_path} に保存しました。")
        print(f"  高度フィルタリング適用。フィルタリング前: {snap4_df.shape[0]}行, フィルタリング後: {filtered_df.shape[0]}行")
    else:
        print(f"  警告: 未知または削除されたフィルタリング手法 '{method}' です。フィルタリングは行われません。")
...
def stage_advanced_filtering(df_input_orig, n_interval, m_interval, delta_tw, target_extraction_percentage):
    ...
    for current_tw in tqdm(unique_tws, desc="  Calculating Ds (spatial gradient) for each tw", leave=False):
        ...
        if sliprate_grid.empty or sliprate_grid.shape[0] < 2 or sliprate_grid.shape[1] < 2: 
            ...
            continue
        if n_interval == 0 or m_interval == 0: # ゼロ除算回避
    ...
    for metric in metrics_to_normalize:
        ...
        if df_input[metric].min() == df_input[metric].max():
            df_input[col_name_norm] = 0.5
        else:
...
def stage_select_variables(df_snap_original, snap4_filtered, method):
    """使用変数決定 (注意箇所1を考慮)"""
    print(f"\n--- 3. 使用変数決定 ---")
    print(f"手法: {method}")
    ...
    if method.startswith("matrix_components_"):
        ...
        if suffix_key in common_cols_suffix:
            ...
            merged_df = pd.merge(snap4_temp_keys, 
                                 ...
                                 on=['n', 'm', 'tw'], how='left')
            final_cluster_cols = base_matrix_cols[:] # コピーを作成
            if 'sliprate' in additional_cols_keys and 'sliprate' in merged_df.columns:
                final_cluster_cols.append('sliprate')
            if 'n' in additional_cols_keys: final_cluster_cols.append('n')
            if 'm' in additional_cols_keys: final_cluster_cols.append('m')
            if 'tw' in additional_cols_keys: final_cluster_cols.append('tw')
        ...
        if suffix_key in common_cols_suffix:
            additional_cols_keys = common_cols_suffix[suffix_key] 
            columns_to_cluster = base_nv6_cols[:] # コピーを作成
...
def stage_pca(data_scaled, method, pca_threshold_config=0.9):
    print(f"\n--- 5. 主成分分析 (PCA) ---")
    print(f"手法: {method}")
    pca_model = None
    ...
    if method == 'pca_none':
        ...
        if data_for_pca_fit.shape[1] < 2: 
            print(f"  警告: データ列が1つ以下のためPCAは実行できません (列数: {data_for_pca_fit.shape[1]})。PCAをスキップします。")
            return data_scaled, None
        pca_temp = PCA()
        pca_temp.fit(data_for_pca_fit)
        cumulative_variance = np.cumsum(pca_temp.explained_variance_ratio_)
        current_pca_threshold = 0.80 if method == 'pca_80' else 0.90
        n_components = np.argmax(cumulative_variance >= current_pca_threshold) + 1
        print(f"  累積寄与率 {current_pca_threshold*100:.0f}% を満たす主成分数: {n_components}")
        n_components = max(1, min(n_components, data_for_pca_fit.shape[1]))
...
def stage_determine_cluster_threshold(linkage_matrix, data_for_silhouette, method, distance_metric_for_silhouette, output_dir, config=None): 
    """クラスターの距離しきい値決定"""
    ...
    if method == 'threshold_arbitrary':
        ...
        upper_k_limit = min(16, actual_data_for_sil_values.shape[0]) 
        if upper_k_limit <=2 : 
             raise ValueError(f"データ点数が非常に少ないため({actual_data_for_sil_values.shape[0]})、シルエット分析で複数のkを試せません。")
        cluster_range = range(2, upper_k_limit) 
        sil_scores = []
        for k_val in tqdm(cluster_range, desc="Silhouette Analysis", leave=False):
            try:
        ...
        ax_sil.plot(list(cluster_range), sil_scores, marker='o')
        ax_sil.set_xlabel("クラスタ数 (k)")
        ax_sil.set_ylabel("シルエットスコア")
        ax_sil.set_title("クラスタ数ごとのシルエットスコア")
        ax_sil.grid(True)
        plt.tight_layout()
        ...
        optimal_k = list(cluster_range)[optimal_k_idx] 
    ...
    # threshold_elbow, threshold_hybrid は削除
    else:
        raise ValueError(f"未知または削除されたしきい値決定手法: {method}")
...
def stage_assign_clusters_and_save_results(
    ...
    if num_original_samples_in_clustering > 0: 
        ...
        dendrogram_final_path = os.path.join(output_dir, "dendrogram_final.pdf")
...
# --- メイン処理関数 ---
def run_single_pipeline(config, pipeline_run_count=None): 
    ...
    df_processed_adv_filter_details = None # 高度フィルタリングの詳細情報DFを保持する変数
    
    try:
        df_snap_orig, snap4_orig = stage_load_data(
            config['working_directory'],
            config.get('snap_y_filename', "snap_y.dat"),
            config.get('n_vector_filename', "n_vector.dat")
        )
        ...
        if config['filter_method'] == 'advanced_filtering':
            ...
            # 高度フィルタリングを実行し、詳細情報DFも取得
            df_processed_adv_filter_details, snap4_filtered = stage_advanced_filtering(
                snap4_orig.copy(),
                advanced_filter_params_dict['n_interval'],
                advanced_filter_params_dict['m_interval'],
                advanced_filter_params_dict['delta_tw'],
                advanced_filter_params_dict['target_extraction_percentage']
            )
        else: # 他のフィルタリング手法
        ...
        data_pca, pca_model = stage_pca(
            data_std.copy(), config['pca_method']
        )
        ...
        if is_all_combinations_mode and config.get('output_directory_path_override'):
            ...
            temp_output_dir_for_thresh_stage = current_run_output_dir 
        else: 
            temp_output_dir_for_thresh_stage = os.path.join(config['working_directory'], "_temp_thresh_artifacts")
            ensure_dir(temp_output_dir_for_thresh_stage)
        ...
        dist_thresh, user_arb_thresh_val = stage_determine_cluster_threshold(
            ...
            output_dir=temp_output_dir_for_thresh_stage, 
            config=config 
        )
        if dist_thresh is None: 
            print("しきい値決定がキャンセルされたか失敗したため、パイプラインを終了します。")
            if not is_all_combinations_mode and os.path.exists(temp_output_dir_for_thresh_stage): 
                try: shutil.rmtree(temp_output_dir_for_thresh_stage)
        ...
        if not is_all_combinations_mode: 
            ...
            current_run_output_dir = os.path.join(config['working_directory'], run_specific_dirname)
            ...
            ensure_dir(current_run_output_dir)
            print(f"結果出力先 (通常モード): {current_run_output_dir}")
            
            if os.path.exists(temp_output_dir_for_thresh_stage) and temp_output_dir_for_thresh_stage != current_run_output_dir:
                try: shutil.rmtree(temp_output_dir_for_thresh_stage)
                except OSError as e: print(f"一時ディレクトリ削除エラー: {e}")
        
        # 高度フィルタリングの詳細情報を最終ディレクトリに保存
        if df_processed_adv_filter_details is not None and current_run_output_dir:
            adv_details_path = os.path.join(current_run_output_dir, "advanced_filtering_details.csv")
            df_processed_adv_filter_details.to_csv(adv_details_path, index=False)
            print(f"  高度フィルタリングの詳細情報を {adv_details_path} に保存しました。")
    ...
    except NotImplementedError as e: # この軽量版では発生しないはず
...
def get_user_config(default_wd):
    config = {}
    print("\n--- クラスタリングパイプラインの設定 ---")
    config["working_directory"] = input(f"1. 作業ディレクトリ (デフォルトの場合Enter) (デフォルト: {default_wd}): ") or default_wd
    while not os.path.isdir(config["working_directory"]):
        print(f"エラー: 指定された作業ディレクトリ '{config['working_directory']}' は存在しないか、ディレクトリではありません。")
        config["working_directory"] = input(f"   有効な作業ディレクトリを入力してください (例: {default_wd}): ")
    ...
    print("\n2. データフィルタリング手法:")
    # skewness_kurtosis を削除
    filter_options = {'1': 'arbitrary_percentage', '2': 'advanced_filtering'} 
    [print(f"  {k}: {v}") for k, v in filter_options.items()] # 未実装表示を削除
    ...
    if config['filter_method'] == 'arbitrary_percentage':
        while True:
            q_val_str = input("   スリップレートのパーセンタイル閾値 (0.0-1.0, 例: 0.75 で上位25%を選択, デフォルト0.75): ") or "0.75"
            try:
        ...
        while True:
            perc_str = input("   抽出したい主要データの割合 (0.0-1.0, 例: 0.25 で上位25%, デフォルト0.25): ") or "0.25"
            try:
    ...
    choice = input(f"   選択 (1-{len(var_options)}, デフォルト{default_var_choice}): ") or default_var_choice
    ...
    print("\n5. 主成分分析 (PCA) 手法:")
    pca_options = {'1': 'pca_80', '2': 'pca_90', '3': 'pca_none'}
    [print(f"  {k}: {v.replace('pca_', '累積寄与率').replace('none','なし') if 'none' not in v else 'なし'}{'%' if 'none' not in v else ''}") for k, v in pca_options.items()]
    choice = input("   選択 (1-3, デフォルト2): ") or '2'
    ...
    print("\n7. クラスター距離しきい値決定手法:")
    # threshold_elbow, threshold_hybrid を削除
    thresh_options = {'1': 'threshold_arbitrary', '2': 'threshold_silhouette'}
    for k, v_raw in thresh_options.items():
        v_display = v_raw.replace('threshold_','').replace('_',' ').capitalize()
        print(f"  {k}: {v_display}") # 未実装表示を削除
    choice = input("   選択 (1-2, デフォルト2): ") or '2'
    config['threshold_method'] = thresh_options.get(choice, 'threshold_silhouette')
    return config

def run_all_combinations(base_working_dir):
    ...
    with tqdm(total=total_combinations, desc="All Combinations Progress") as pbar:
        for i, config_item in enumerate(param_combinations):
            ...
            try:
                run_single_pipeline(config_item, pipeline_run_count=run_count_str)
            # NotImplementedError は発生しないはずなので、キャッチブロックは不要
...
def main():
    print("メイン関数が開始されました。") 
    default_working_directory = "/Users/harashima-yukio/Desktop/2024_Noto/results_20240702-182819/" 
    # default_working_directory = "./test_data/" # ローカルテスト用

    if not os.path.isdir(default_working_directory):
        print(f"警告: デフォルトの作業ディレクトリ '{default_working_directory}' が見つかりません。")
        manual_wd = input("有効な作業ディレクトリパスを入力してください (例: ./data/ ): ")
        if not os.path.isdir(manual_wd): print("エラー: 有効な作業ディレクトリが指定されませんでした。終了します。"); return
        default_working_directory = manual_wd
    print(f"使用する作業ディレクトリ: {default_working_directory}")
    ...
    if run_mode == '1':
        user_config = get_user_config(default_working_directory)
        run_single_pipeline(user_config)
    elif run_mode == '2':
        configs_to_run = [ 
            {
                "working_directory": default_working_directory, "filter_method": "arbitrary_percentage", 
        ...
        for i, cfg in enumerate(configs_to_run):
            ...
            run_single_pipeline(cfg, pipeline_run_count=f"batch_{i+1:02d}")
    elif run_mode == '3':
        run_all_combinations(base_working_dir=default_working_directory)
    else:
