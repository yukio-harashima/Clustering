...
# --- Generate new time window definition file for swap_mmcl5_revised_v2.py ---
...
if [ ${num_new_tw} -le 0 ] && ! [ -f ${new_tw_file} ]; then
    ...
    touch ${new_tw_file} # Create an empty file to avoid swap_mmcl5_revised_v2.py error
fi

# --- Execute swap_mmcl5_revised_v2.py (assuming it's in the PATH or same directory) ---
export CLNO=${clno} 
echo "情報: swap_mmcl5_revised_v2.py を実行します..."
if command -v swap_mmcl5_revised_v2.py &> /dev/null; then
    swap_mmcl5_revised_v2.py
    if [ $? -ne 0 ]; then
        echo "エラー: swap_mmcl5_revised_v2.py の実行に失敗しました。ログを確認してください。"
        exit 1
    fi
else
    echo "エラー: swap_mmcl5_revised_v2.py が見つかりません。"
    exit 1
fi
echo "情報: swap_mmcl5_revised_v2.py の実行が完了しました。"
...
#--- Begin GMT plotting
# ===== 変更点: 出力ファイル名を変更 =====
output_filename="ffi_MapXCSnap_v3_${i1}_cl${clno}"
if [ $# -eq 5 ]; then
    output_filename+="_start${i4}_end${i5}"
fi
# output_filename+=".png"
processed_plot_data_file="clusteringSnap2_v2.dat" # swap_mmcl5_revised_v2.py output

if [ ! -f ${processed_plot_data_file} ]; then
    echo "エラー: ${processed_plot_data_file} が見つかりません。swap_mmcl5_revised_v2.pyが正しく実行されたか確認してください。"
...
#   gmt subplot begin ${row}x${column} -Fs${mapwidth}c/${mapheight}c -M0.1c/1.0c -SCb -SRl -Bwsne -R${r} -JM${mapwidth}c -Ba1f0.5
  gmt subplot begin ${row}x${column} -Fs${mapwidth}+c2 -M0.1/1.0 -SCb -SRl -Bwsne -R${r} -JM${mapwidth} -Ba1
    ...
    while IFS=" " read -r current_tw_for_plot start_time_label end_time_label; do
        ...
        gmt grdimage dem.nc -Igrad.grd -Ctopo.cpt -t40
        ...
        # plot strike
        # awk -F'\t' -v tw_val="${current_tw_for_plot}" -v clnum="${clno}" \
        #     '$3==tw_val && $30==clnum && $6> 0.00001{print $20,$19,$13,$6*3}' tmp.dat \
        #     | gmt plot -SV0c+jc -W0.3p,black -t80
        ...
        awk -F'\t' -v tw_val="${current_tw_for_plot}" -v clnum="${clno}" \
            ...
            | gmt plot -SV0c+jc -W0.3+cl -Cslip.cpt -t40
