...
#---
if [ ! -e plot_cwave_f.list ] && [ -e ./Wave_cp/plot_cwave_f.list ]; then
  cp ./Wave_cp/plot_cwave_f.list  .
fi
if [ ! -e plot_cwave_f.list ] && [ -e ./Wave_cp/plot_cwave_f.csh ]; then
  cp ./Wave_cp/plot_cwave_f.csh  plot_cwave_f.list
fi
#---
...
for station in "${st_list[@]}"; do
  info=($(gmt gmtinfo ./Wave_cp/${station}"BHZ_obs" -C))
...
column=2; row=$(echo "scale=2;${num_station}/${column}+0.99" | bc | awk '{printf("%d\n", $1)}')
...
gmt begin ffi_WaveComp png
  gmt subplot begin ${row}x${column} -Fs${mapwidth}/${mapheight} -M${quarter}/0.0 -B -R${r} -JX${mapwidth}/${mapheight}
    for station in "${st_list[@]}"; do
      ...
      gmt subplot set
      #---
      gmt plot ./Wave_cp/${station}"BHZ_obs"  -W0.8p
      gmt plot ./Wave_cp/${station}"BHZ_syn" -W0.8p,red -N
      #---
...
gmt end show
echo -e " done !"
if [ -e ./Wave_cp/wave.obs_f ]; then  rm -rf wave.obs_f ;fi
if [ -e ./Wave_cp/wave.syn   ]; then  rm -rf wave.syn   ;fi
