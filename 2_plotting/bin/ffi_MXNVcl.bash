...
#--- Begin GMT plotting
# ===== 変更点: 出力ファイル名を変更 =====
output_filename="ffi_MapXnvSnap_${i1}_cl${clno}"
if [ $# -eq 5 ]; then
...
#   gmt subplot begin ${row}x${column} -Fs${mapwidth}c/${mapheight}c -M0.1c/1.0c -SCb -SRl -Bwsne -R${r} -JM${mapwidth}c -Ba1f0.5
  gmt subplot begin ${row}x${column} -Fs${mapwidth}+c2 -M0.1/1.0 -SCb -SRl -Bwsne -R${r} -JM${mapwidth} -Ba1
    ...
    while IFS=" " read -r current_tw_for_plot start_time_label end_time_label; do
        ...
        # # plot strike
        # awk -F'\t' -v tw_val="${current_tw_for_plot}" -v clnum="${clno}" \
        #     '$3==tw_val && $30==clnum && $6> 0.00001{print $20,$19,$30,$13,1}' tmp.dat \
        #     | gmt plot -SVz0c+jc -W1.5+cl -C
        
        #  awk -F'\t' -v tw_val="${current_tw_for_plot}" -v clnum="${clno}" \
        #     '$3==tw_val && $30==clnum && $6> 0.00001{print $20,$19,$30,$14,1}' tmp.dat \
        #     | gmt plot -SVz0c+jc -W1.5+cl -C
        ...
        # plot nv vector
        # 1本目の線をプロット (走向1: $13, 傾斜1: $15)
        # awk -F'\t' -v tw_val="${current_tw_for_plot}" -v clnum="${clno}" '
        #     $3==tw_val && $30==clnum && $6 > 0.00001 {
        #         PI = atan2(0, -1);
        #         strike_rad = $13 * PI / 180;
        #         dip_rad = $15 * PI / 180;
        #         x_comp = sin(dip_rad) * cos(strike_rad);
        #         y_comp = -sin(dip_rad) * sin(strike_rad);
        #         print $20, $19, x_comp, y_comp, $30;
        #     }' tmp.dat \
        #     | gmt plot -Svs+e+jc -W1.5+cl -C

        # # 2本目の線をプロット (走向2: $14, 傾斜2: $16)
        # awk -F'\t' -v tw_val="${current_tw_for_plot}" -v clnum="${clno}" '
        #     $3==tw_val && $30==clnum && $6 > 0.00001 {
        #         PI = atan2(0, -1);
        #         strike_rad = $14 * PI / 180;
        #         dip_rad = $16 * PI / 180;
        ...
        awk -F'\t' -v tw_val="${current_tw_for_plot}" -v clnum="${clno}" '
            $3==tw_val && $30==clnum && $6 > 0.00001 {
                # awkのsin/cosはラジアンを引数に取るため、度からラジアンに変換します。
                PI = atan2(0, -1);
                strike_rad = $13 * PI / 180;
                dip_rad = $15 * PI / 180;

                # 指示された計算式で東西成分(x)と南北成分(y)を求めます。
                x_comp = sin(dip_rad) * cos(strike_rad);
                y_comp = -sin(dip_rad) * sin(strike_rad);

                # GMTに渡すデータ: 経度($20), 緯度($19), x成分, y成分, クラスタ番号($30)
                print $20, $19, x_comp, y_comp, $30;
            }' tmp.dat \
            | gmt plot -Sv0.4c+e+jc -W1.5+cl -C
