...
gmt set FONT_LABEL 23p
gmt set FONT_ANNOT_PRIMARY 23p
...
gmt begin ffi_MapMeca png
  ...
  if [ ! -e "zeroone.txt" ]; then
    ...
    for n in $(seq 1 ${nn} | sort -nr); do
      for m in $(seq 1 ${mn} ); do
        if [ $(awk '{if (NR=='$((${nn}+1-$n))') print $'$m'}' zeroone.txt) -eq 1 ]; then
          #--- circle
          # awk '{if($1=='$n' && $2=='$m') print $4,$3}' knotplot.dat | gmt plot -Sp0.08 -W1,white
          #--- outline
          if [ $m -eq 1 ] || [ $(awk '{if (NR=='$((${nn}+1-$n))') print $'$(($m-1))'}' zeroone.txt) -eq 0 ]; then # left
            gmt plot -W1 << END
  $(awk '{if($1=='$n' && $2=='$m') print $4,$3}' knotcorner.dat)
  $(awk '{if($1=='$n' && $2=='$m') print $7,$6}' knotcorner.dat)
END
          fi
          if [ $m -eq ${mn} ] || [ $(awk '{if (NR=='$((${nn}+1-$n))') print $'$(($m+1))'}' zeroone.txt) -eq 0 ]; then # right
            gmt plot -W1 << END
  $(awk '{if($1=='$n' && $2=='$m') print $10,$9}' knotcorner.dat)
  $(awk '{if($1=='$n' && $2=='$m') print $13,$12}' knotcorner.dat)
END
          fi
          if [ $n -eq ${nn} ] || [ $(awk '{if (NR=='$((${nn}-$n))') print $'$m'}' zeroone.txt) -eq 0 ]; then # top
            gmt plot -W1 << END
  $(awk '{if($1=='$n' && $2=='$m') print $7,$6}' knotcorner.dat)
  $(awk '{if($1=='$n' && $2=='$m') print $10,$9}' knotcorner.dat)
END
          fi
          if [ $n -eq 1 ] || [ $(awk '{if (NR=='$((${nn}+2-$n))') print $'$m'}' zeroone.txt) -eq 0 ]; then # bottom
            gmt plot -W1 << END
  $(awk '{if($1=='$n' && $2=='$m') print $4,$3}' knotcorner.dat)
  $(awk '{if($1=='$n' && $2=='$m') print $13,$12}' knotcorner.dat)
END
