...
#---
mecascale=6      # To change the size of the mechanism solution, change the value of mecascale.
#---
...
# ユーザーが bash MapMecaSnap.bash 10 100 0 のように3つの数字（間隔、合計時間、モード）を指定しなかった場合、自動設定モードに入ります。
# 指定した場合は else 以下の手動モードに行きます。
if [ $# -ne 3 ]; then
  ...
  echo ${interval}  ${tmax}  0  | fgensnap_mf  > .tmpout  ; rm -rf .tmpout
  i1=${interval}; i2=${tmax}; i3=0
else
  echo ${1} ${2} ${3} | fgensnap_mf
  i1=${1}; i2=${2}; i3=${3}
fi
#---
...
gmt begin ffi_MapMecaSnap_Mf png
  ...
  gmt subplot begin ${row}x${column} -Fs${mapwidth} -M0.10/0.3 -SCb -SRl -Bxa0.5 -Bya0.5 -BWSne -R${r} -JM${mapwidth}
    for tw in $(seq 1 ${num_tw}); do
      ...
      awk '{if ($3=='${tw}' &&  $6 > 0.00001) print $20,$19,$6,$7,$8,$9,$10,$11,$12,23}' snap.dat |sort -k 3 -n  | gmt meca -Sm${mecasize}c+l -T0 -Cslip.cpt
      #---
...
#      if [ ${dip} -gt 0 ]; then sort -k 3 -n faultline.dat | awk 'NR <= 2 {print $2,$1}' | gmt plot -W1 ; fi
      ...
      #---
      if [ ${tw} -eq ${num_tw} ]; then gmt colorbar -Cslip.cpt -DjBR+w${mapheight}/0.2+jBL+o0.05/0.0 -Ba${half}+l"Potency-rate Density (m/s)"; fi
    done
  gmt subplot end
