...
# calc_fault_v4.pyと gen_schmidt_pr_v2.pyを実行した後に行うように

# --- 設定 ---
# 入力データファイル名
INFILE="vec_inner_sch.dat"
# 出力画像ファイル名
OUTFILE="ffi_schmidt_plot_inner"
# 図の縮尺 (1単位あたりのcm)
SCALE=5
...
gmt makecpt -Cviridis -T0/${slipmax} -I --COLOR_NAN=white --COLOR_BACKGROUND=white --COLOR_FOREGROUND=white > slip_color.cpt
...
gmt begin $OUTFILE
  ...
  gmt subplot begin 1x2 -Fs12c/12c -M0.1c -R-1.2/1.2/-1.2/1.2 -B+n
    ...
    awk -F'\t' '$8 > 0 {print $4, $5, $9}' vec_inner_sch.dat | \
    gmt plot -Sc0.2c -W0.2p,black -Cslip_color.cpt

    # モデル面のプロット
    awk -F'\t' '{print $4, $5}' model_vec_sch.dat | gmt plot -Sa0.2c -W0.2p,black -Gred
    ...
    awk -F'\t' '$8 > 0 {print $6, $7, $10}' vec_inner_sch.dat | \
  ...
  # カラーバー (共通)
  # 図の下中央に配置
  gmt colorbar -Cslip_color.cpt -DJBC+w15c/0.3c+o0c/0c+h -Baf -B+l"vector inner"
