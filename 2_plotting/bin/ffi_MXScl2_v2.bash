...
# --- Generate new time window definition file for swap_mmcl5_fortran_call.py ---
...
if [ ${num_new_tw} -le 0 ] && ! [ -f ${new_tw_file} ]; then
    ...
    touch ${new_tw_file} # Create an empty file to avoid swap_mmcl5_fortran_call.py error
fi

# --- Execute swap_mmcl5_fortran_call.py (assuming it's in the PATH or same directory) ---
export CLNO=${clno} 
echo "情報: swap_mmcl5_fortran_call.py を実行します..."
if command -v swap_mmcl5_fortran_call.py &> /dev/null; then
    swap_mmcl5_fortran_call.py
    if [ $? -ne 0 ]; then
        echo "エラー: swap_mmcl5_fortran_call.py の実行に失敗しました。ログを確認してください。"
        exit 1
    fi
else
    echo "エラー: swap_mmcl5_fortran_call.py が見つかりません。"
    exit 1
fi
echo "情報: swap_mmcl5_fortran_call.py の実行が完了しました。"
...
# --- 変数設定 ---
# 断層データが格納されているフォルダ
FAULT_DIR="/Users/harashima-yukio/Desktop/JpGU2025/prz/fig/hk/fault_dat"
# 震源情報 (スクリプトの引数から取得)
EPI_LAT=$1
EPI_LON=$2
# 出力ファイル名
...
#--- Begin GMT plotting
# ===== 変更点: 出力ファイル名を変更 =====
output_filename="ffi_MapXCSnap_v2_${i1}_cl${clno}"
if [ $# -eq 5 ]; then
    output_filename+="_start${i4}_end${i5}"
fi
# output_filename+=".png"
processed_plot_data_file="clusteringSnap2_v2.dat" # swap_mmcl5_fortran_call.py output

if [ ! -f ${processed_plot_data_file} ]; then
    echo "エラー: ${processed_plot_data_file} が見つかりません。swap_mmcl5_fortran_call.pyが正しく実行されたか確認してください。"
...
clinfo=($(gmt gmtinfo clusteringSnap2_v2.dat -C))
# echo "clinfo ${clinfo[@]}"
echo "slipmax ${clinfo[11]}"
# slipmax=${info[11]}
cslipmax=${clinfo[11]}
...
# 断層すべり用のCPTファイルを作成
  gmt makecpt -Cvik -T0.5/${labmax}/1 -I --COLOR_NAN=white --COLOR_BACKGROUND=white --COLOR_FOREGROUND=white > slip.cpt
#   gmt makecpt -CvikO -T0.5/${labmax}/1 -I --COLOR_NAN=white --COLOR_BACKGROUND=white --COLOR_FOREGROUND=white > slip.cpt
...
#   gmt makecpt -CvikO -T0.5/${labmax}/1 -I --COLOR_NAN=white --COLOR_BACKGROUND=white --COLOR_FOREGROUND=white
#   gmt subplot begin ${row}x${column} -Fs${mapwidth}c/${mapheight}c -M0.1c/1.0c -SCb -SRl -Bwsne -R${r} -JM${mapwidth}c -Ba1f0.5
  gmt subplot begin ${row}x${column} -Fs${mapwidth}+c2 -M0.1/1.0 -SCb -SRl -Bwsne -R${r} -JM${mapwidth} -Ba1
    ...
    while IFS=" " read -r current_tw_for_plot start_time_label end_time_label; do
        ...
        # awk -F'\t' -v tw_val="${current_tw_for_plot}" -v clnum="${clno}" \
        #     '$3==tw_val && $30==clnum && $6> 0.00001{print $20,$19,$30,$7,$8,$9,$10,$11,$12,23}' tmp.dat \
        #     | gmt meca -Sm${mecasize}c+l -T0 -C -N

        # plot strike
        awk -F'\t' -v tw_val="${current_tw_for_plot}" -v clnum="${clno}" \
            '$3==tw_val && $30==clnum && $6> 0.00001{print $20,$19,$13,$6/'${cslipmax}'*1.57}' tmp.dat \
            | gmt plot -SV0c+jc -W1.2p,black -t60
                
        # #  awk -F'\t' -v tw_val="${current_tw_for_plot}" -v clnum="${clno}" \
        #     # '$3==tw_val && $30==clnum && $6> 0.00001{print $20,$19,$14,$6*1.2}' tmp.dat \
        #     # | gmt plot -SV0c+jc -W2.5p,black

        awk -F'\t' -v tw_val="${current_tw_for_plot}" -v clnum="${clno}" \
            '$3==tw_val && $30==clnum && $6> 0.00001{print $20,$19,$30,$13,$6/'${cslipmax}'*1.5}' tmp.dat \
            | gmt plot -SV0c+jc -W0.8+cl -Cslip.cpt -t20
        
        #  awk -F'\t' -v tw_val="${current_tw_for_plot}" -v clnum="${clno}" \
        #     '$3==tw_val && $30==clnum && $6> 0.00001{print $20,$19,$30,$14,$6}' tmp.dat \
        #     | gmt plot -SV0c+jc -W1.5+cl -C

        # # plot strike
        # awk '{if ($3=='${tw}') print $20,$19,$6,$13,$6/'${slipmax}'*0.4}' tmp.dat | gmt plot -SV0c+jc -W1.5+cl -C
        # awk '{if ($3=='${tw}') print $20,$19,$6,$14,$6/'${slipmax}'*0.4}' tmp.dat | gmt plot -SV0c+jc -W1.5+cl -C
