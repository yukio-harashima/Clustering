...
echo info: ${info[0]}, ${info[1]}, ${info[2]}, ${info[3]} 

#+++++  2025/3/30 Yuji Yagi  ++++++++　7/16 modernized harahima
# gold ratio
# widthratio=1.618034
# # shilver ratio
widthratio=1.414235
hfratio=$(echo "${widthratio}*0.5" |bc)
...
if [ $result -eq 1 ]; then
  wk_x=$(echo "scale=5;(${DY}-${DX})*0.5+(${DY}*0.025)"|bc)
  # wk_y=$(echo "(${DX}+${wk_x})*${widthratio}-${DY}"|bc)
  wk_y=$(echo "scale=5;((${DX}+${wk_x}*2)*${hfratio})-(${DY}*0.5)"|bc)
  X=$(echo "scale=5;${DX}+${wk_x}*2" | bc)
  Y=$(echo "scale=5;${DY}+${wk_y}*2"| bc)
  YX=$(echo "scale=5;${Y}/${X}"| bc)
  if [ $(echo "${wk_x} < 0" | bc) -eq 1 ]; then
    wk_x=$(echo "${wk_x} * -1" | bc)
  fi

  if [ $(echo "${wk_y} < 0" | bc) -eq 1 ]; then
    wk_y=$(echo "${wk_y} * -1" | bc)
  fi
  echo wk_x:${wk_x}, wk_y:${wk_y}, X:${X}, Y:${Y}, YX:${YX} 
else
  wk_y=$(echo "scale=5;(${DX}-${DY})*0.5+(${DX}*0.025)"|bc)
  wk_x=$(echo "scale=5;((${DY}+${wk_y}*2)*${hfratio})-(${DX}*0.5)"|bc)
  X=$(echo "scale=5;${DX}+${wk_x}*2" | bc)
  Y=$(echo "scale=5;${DY}+${wk_y}*2"| bc)
  XY=$(echo "scale=5;${X}/${Y}"| bc)
  if [ $(echo "${wk_x} < 0" | bc) -eq 1 ]; then
    wk_x=$(echo "${wk_x} * -1" | bc)
  fi
  ...
  echo wk_x:${wk_x}, wk_y:${wk_y}, X:${X}, Y:${Y}, XY:${XY}
fi
