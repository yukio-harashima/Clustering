...
その後、代表クラスタ番号で抽出されたデータ内で、同じグリッド点にデータが複数残っている場合（通常は元の細かい時間ステップで代表クラスタが複数回出現した場合）、sliprate が最大のものを残します。
...
def revised_swap_process():
    ...
    for _, new_window_row in new_time_windows_df.iterrows():
        ...
        for grid_keys, group_at_grid in data_in_current_new_window.groupby(['n', 'm']):
            ...
            if data_with_representative_no.empty:
                continue

            data_with_representative_no_no_nan_sliprate = data_with_representative_no.dropna(subset=['sliprate'])
            if not data_with_representative_no_no_nan_sliprate.empty:
                idx_max_sliprate = data_with_representative_no_no_nan_sliprate['sliprate'].idxmax()
                selected_row = data_with_representative_no_no_nan_sliprate.loc[idx_max_sliprate].copy()
                selected_rows_for_this_new_tw.append(selected_row)
            elif not data_with_representative_no.empty:
                selected_rows_for_this_new_tw.append(data_with_representative_no.iloc[0].copy())
...
if __name__ == '__main__':
    print("swap_mmcl5_revised.py (v3) を実行中...")
