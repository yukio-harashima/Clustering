...
program fgensnap_y
  ...
  open(31, file = 'snap2_y.dat', status = 'replace')
...
!           if (sliprate(m, n, tw) .ne. 0)then
           if (sliprate(m, n, tw) .gt. 0.0000001)then
              ...
              !--- t, x, y, n1_vector(1,2,3), n2_vector(1,2,3)の計算
              ...
              write(34,'(3i5,11f12.5)') n, m, tw, real(tw)/num_timewindow, & 
                   mtx(m)/(maxval(mtx)-minval(mtx)), mty(n)/(maxval(mtx)-minval(mtx)), &
  ...
  do tw = 1, num_timewindow
     ...
     do m = 1, mn
        do n = 1, nn
           moth = moth + sliprate(m, n, tw)
           ch = ch + sliprate(m, n, tw) * mtx(m)
        end do
     end do
     x = ch / moth
     ch = 0.
     do m = 1, mn
        do n = 1, nn
           ch = ch + sliprate(m, n, tw) * mty(n)
        end do
     end do
     y = ch / moth
     nexpo = int(log10(maxval(abs(mrf))))
     mrf = mrf / (10.**nexpo)
     call xy2geo(epilat, epilon, x, y, strike0, dip0, lat, lon)
     dep = epidep - y * sin(dip0 * rad)
  ...
  !--- for snap shot-2 ( slip distribution ) ---
  x1 = -xx * real(m0)
  x2 = xx * (real(mn - m0) + 1.0)
  y1 = -yy * real(n0)
  if (ns == 1 .or. ns == 1)then
     y2 = yy * (real(nn-n0) )  + ylen
  else
     y2 = yy * (real(nn-n0) + 1.0 )
  endif
  nx = int((x2 - x1)* 2. + 2)
  ny = int((y2 - y1)* 2. + 2)
  dx = 0.5 ; dy = 0.5
  ...
  do tw = 1, num_timewindow
     do n = 1, ny
        do m = 1, nx
           x = int(x1 * 5)/5 + dx * (m - 1)
           y = int(y1 * 5)/5 + dy * (n - 1)
           wk = slip_xy(sliprate(0:mn+2, 0:nn+2, tw), mn, nn, m0, n0, xx, yy, x, y)
           if (wk < 0)then
              wk = 0
           end if
           call xy2geo(epilat, epilon, x, y, strike0, dip0, lat, lon)
           dep = epidep - y * sin(dip0 * rad)
           write(31,'(3i5,2f11.3,f11.6, 3f9.3)')n, m, tw, x, y, wk, lat, lon, dep
           if (lon < 0.) then
             lon=360.+lon
           end if
           if     (lat <= latmin) then
           ...
           elseif (lat > latmax) then
              ...
              lon_n  = lon
              n_depth = dep
           elseif (lon <= lonmin) then
              lonmin = lon
              lat_w  = lat
              lon_w  = lon
              w_depth = dep
           elseif (lon > lonmax) then
              lonmax = lon
              lat_e  = lat
              lon_e  = lon
              e_depth = dep
  ...
  if (mod(strike0, 90.) == 0) then
     write(33, '(3f9.3)')lat_n, lon_w, w_depth
     write(33, '(3f9.3)')lat_n, lon_e, e_depth
     write(33, '(3f9.3)')lat_s, lon_e, e_depth
     write(33, '(3f9.3)')lat_s, lon_w, w_depth
     write(33, '(3f9.3)')lat_n, lon_w, w_depth
  else
     write(33, '(3f9.3)')lat_w, lon_w, w_depth
     write(33, '(3f9.3)')lat_n, lon_n, n_depth
     write(33, '(3f9.3)')lat_e, lon_e, e_depth
     write(33, '(3f9.3)')lat_s, lon_s, s_depth
     write(33, '(3f9.3)')lat_w, lon_w, w_depth
  endif
  close(33)
  !---
